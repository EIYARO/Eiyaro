...
func initNativeAsset() {
	...
	alias := consensus.EYAlias

	definitionBytes, _ := serializeAssetDef(consensus.EYDefinitionMap)
	DefaultNativeAsset = &Asset{
		Signer:            signer,
		AssetID:           *consensus.EYAssetID,
		Alias:             &alias,
		VMVersion:         1,
		DefinitionMap:     consensus.EYDefinitionMap,
...
var (
	...
	ErrInternalAsset  = errors.New("ey has been defined as the internal asset")
...
func (reg *Registry) UpdateAssetAlias(id, newAlias string) error {
	...
	if oldAlias == consensus.EYAlias || newAlias == consensus.EYAlias {
