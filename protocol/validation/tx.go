...
func (g *GasState) setGas(EYValue int64, txSize int64) error {
	if EYValue < 0 {
		return errors.Wrap(ErrGasCalculate, "input EY is negative")
	}

	g.EYValue = uint64(EYValue)

	var ok bool
	if g.GasLeft, ok = checked.DivInt64(EYValue, consensus.VMGasRate); !ok {
...
func checkValid(vs *validationState, e bc.Entry) (err error) {
	...
	case *bc.Mux:
		...
		for assetID, amount := range parity {
			if assetID == *consensus.EYAssetID {
		...
		for _, EYInputID := range vs.tx.GasInputIDs {
			e, ok := vs.tx.Entries[EYInputID]
			if !ok {
				return errors.Wrapf(bc.ErrMissingEntry, "entry for eiyaro input %x not found", EYInputID)
			}

			vs2 := *vs
			vs2.entryID = EYInputID
	...
	case *bc.Coinbase:
		...
		if *e.WitnessDestination.Value.AssetId != *consensus.EYAssetID {
...
func checkStandardTx(tx *bc.Tx, blockHeight uint64) error {
	...
	for _, id := range tx.ResultIds {
		...
		if !ok || *output.Source.Value.AssetId != *consensus.EYAssetID {
			continue
		}
